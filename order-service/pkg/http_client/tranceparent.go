...
func GetTraceparentFromOtelContext(ctx trace.SpanContext) (traceparent string) {
	traceparent = fmt.Sprintf("00-%s-%s-%s", ctx.TraceID().String(), ctx.SpanID().String(), ctx.TraceFlags().String())
	return
}
